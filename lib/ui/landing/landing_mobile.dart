...
class LoginWithMobileWidget extends ConsumerWidget {
  const LoginWithMobileWidget({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final landingMobileClient = ref.watch(landingMobileClientProvider);

    if (landingMobileClient.isLoading) {
      return const Center(child: CircularProgressIndicator());
    }
    if (landingMobileClient.hasError) {
      return const Center(child: CircularProgressIndicator());
    }
    return Navigator(
...
class _PhoneNumberInputScene extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final client = ref.watch(landingMobileClientProvider).value;
    return Column(
      children: [
          ...
          child: PhoneNumberInputLayout(
            onNextStep: (phoneNumber) async {
              final ret = await showConfirmMixinDialog(
                context,
                l10n.landingInvitationDialogContent(phoneNumber),
                maxWidth: 440,
              );
              if (ret == null) return;
              showToastLoading();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> performLogin(String code) async {
      ...
      try {
        ...
        final client = await ref.read(landingMobileClientProvider.future);
            ...
            onResend: () async {
              ...
              try {
                final response = await _requestVerificationCode(
                  ...
                  context: context,
                  accountApi: ref
                      .read(landingMobileClientProvider)
                      .requireValue
                      .accountApi,
                );
                Toast.dismiss();
...
Future<VerificationResponse> _requestVerificationCode({
  ...
  try {
    final response = await accountApi.verification(request);
