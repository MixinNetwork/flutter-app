...
final tokenProvider = StreamProvider.autoDispose.family<Token?, String?>((
  ...
  final stream = database.tokenDao
      .tokenById(assetId)
      .watchSingleOrNullWithStream(
        eventStreams: [
          DataBaseEventBus.instance.updateTokenStream.where(
...
final safeSnapshotProvider = StreamProvider.autoDispose
    .family<SafeSnapshot?, String?>((ref, snapshotId) {
      if (snapshotId == null) {
        return const Stream.empty();
      }
      final database = ref.read(accountServerProvider).requireValue.database;
      final stream = database.safeSnapshotDao
          ...
          .watchSingleOrNullWithStream(
            eventStreams: [
              DataBaseEventBus.instance.updateSafeSnapshotStream.where(
                (event) => event.contains(snapshotId),
              ),
            ],
            duration: kDefaultThrottleDuration,
          );
...
final assetChainProvider = StreamProvider.autoDispose.family<Chain?, String?>((
  ...
  final database = ref.read(accountServerProvider).requireValue.database;
  final stream = database.chainDao
      .chain(assetId)
      .watchSingleOrNullWithStream(
        eventStreams: [
