...
extension EncryptCategoryExtension on EncryptCategory {
  ...
  String asCategory(String category) {
    if (category.isText) {
      return toCategory(MessageCategory.plainText, MessageCategory.signalText,
          MessageCategory.encryptedText);
    }
    if (category.isImage) {
      return toCategory(MessageCategory.plainImage, MessageCategory.signalImage,
          MessageCategory.encryptedImage);
    }
    if (category.isAudio) {
      return toCategory(MessageCategory.plainAudio, MessageCategory.signalAudio,
          MessageCategory.encryptedAudio);
    }
    if (category.isVideo) {
      return toCategory(MessageCategory.plainVideo, MessageCategory.signalVideo,
          MessageCategory.encryptedVideo);
    }
    if (category.isSticker) {
      return toCategory(MessageCategory.plainSticker,
          MessageCategory.signalSticker, MessageCategory.encryptedSticker);
    }
    if (category.isData) {
      return toCategory(MessageCategory.plainData, MessageCategory.signalData,
          MessageCategory.encryptedData);
    }
    if (category.isLive) {
      return toCategory(MessageCategory.plainLive, MessageCategory.signalLive,
          MessageCategory.encryptedLive);
    }
    if (category.isPost) {
      return toCategory(MessageCategory.plainPost, MessageCategory.signalPost,
          MessageCategory.encryptedPost);
    }
    if (category.isLocation) {
      return toCategory(MessageCategory.plainLocation,
