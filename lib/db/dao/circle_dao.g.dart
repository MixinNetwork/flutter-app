...
mixin _$CircleDaoMixin on DatabaseAccessor<MixinDatabase> {
  ...
  Selectable<ConversationCircleItem> allCircles() {
    return customSelect(
      ...
      variables: [],
      readsFrom: {circles, conversations, users, circleConversations},
    ).map(
      (QueryRow row) => ConversationCircleItem(
        ...
        orderedAt: NullAwareTypeConverter.wrapFromSql(
          Circles.$converterorderedAt,
          row.readNullable<int>('ordered_at'),
        ),
  ...
  Selectable<ConversationCircleManagerItem> circleByConversationId(
    ...
    return customSelect(
      ...
      readsFrom: {circles, conversations, circleConversations},
    ).map(
      (QueryRow row) => ConversationCircleManagerItem(
        circleId: row.read<String>('circle_id'),
        name: row.read<String>('name'),
        count: row.read<int>('count'),
      ),
  ...
  CircleDaoManager get managers => CircleDaoManager(this);
}

class CircleDaoManager {
  final _$CircleDaoMixin _db;
  CircleDaoManager(this._db);
