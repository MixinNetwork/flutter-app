...
class ConversationDao extends DatabaseAccessor<MixinDatabase>
  ...
  Expression<bool> _conversationPredicateByCategory(
    ...
    switch (category) {
      ...
      case SlideCategoryType.groups:
        predicate = conversation.category.equalsValue(
  ...
  Selectable<ConversationItem> conversationItems() => _baseConversationItems(
    (
      ...
      em,
        ...
        _conversationPredicateByCategory(SlideCategoryType.chats, conversation),
    (
      conversation,
      owner,
      lastMessage,
  ...
  Future<int> pin(String conversationId) =>
      (update(db.conversations)
            ..where((tbl) => tbl.conversationId.equals(conversationId)))
          .write(ConversationsCompanion(pinTime: Value(DateTime.now())))
          .then((value) {
            if (value > 0) {
    ...
    return db
        .transaction(() async {
          await Future.wait([
            ...
            ...(conversation.participantSessions ?? []).map(
              (p) => db.participantSessionDao.insert(
  ...
  Future<List<Conversation>> getConversations({
    ...
    required int offset,
      ...
      (select(db.conversations)
            ..orderBy([(c) => OrderingTerm.asc(c.rowId)])
