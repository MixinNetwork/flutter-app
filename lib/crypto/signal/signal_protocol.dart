...
import 'package:flutter_app/crypto/signal/vo/encrypt_result.dart';
...
class SignalProtocol {
  ...
  Future initStore() async {
    ...
    final preKeyStore = MixinPreKeyStore(_store);
    final signedPreKeyStore = MixinPreKeyStore(_store);
    final identityKeyStore = MixinIdentityKeyStore(_store, _sessionId);
    final sessionStore = MixinSessionStore(_store);
    mixinSignalProtocolStore = MixinSignalProtocolStore(
        preKeyStore, signedPreKeyStore, identityKeyStore, sessionStore);
    senderKeyStore = MixinSenderKeyStore(_store);
  }

  Store get store => _store;

  Uint8List? getSenderKeyPublic(String groupId, String userId,
      {String? sessionId}) {
    final senderKeyName = SenderKeyName(
        groupId, SignalProtocolAddress(userId, sessionId.getDeviceId()));
    final sender = senderKeyStore.loadSenderKey(senderKeyName);
    try {
  ...
  SenderKeyDistributionMessageWrapper getSenderKeyDistribution(
  ...
  EncryptResult encryptSenderKey(String conversationId, String recipientId,
      {int deviceId = defaultDeviceId}) {
    final senderKeyDistributionMessage =
        getSenderKeyDistribution(conversationId, _sessionId);
    try {
      final cipherMessage = encryptSession(
  ...
  CiphertextMessage encryptSession(
      Uint8List content, String destination, int deviceId) {
  ...
  bool isExistSenderKey(String groupId, String senderId) {
    final senderKeyName = SenderKeyName(
        groupId, SignalProtocolAddress(_sessionId, defaultDeviceId));
    final senderKeyRecord = senderKeyStore.loadSenderKey(senderKeyName);
    return !senderKeyRecord.isEmpty;
  }

  bool containsUserSession(String recipientId) {
    final sessions = mixinSignalProtocolStore.sessionStore.sessionDao
        .getSessions(recipientId);
    return sessions.isNotEmpty;
  }

  bool containsSession(String recipientId, {int deviceId = defaultDeviceId}) {
  ...
  void deleteSession(String userId) {
    mixinSignalProtocolStore.sessionStore.sessionDao.deleteSessions(userId);
  }
  ...
  BlazeMessage encryptSessionMessage(Message message, String recipientId,
      {String? resendMessageId, String? sessionId, List<String>? mentionData}) {
    final cipher = encryptSession(
  ...
  BlazeMessage encryptGroupMessage(Message message, List<String>? mentionData) {
  ...
  Uint8List decryptGroupMessage(String groupId, SignalProtocolAddress address,
      Uint8List cipherText, DecryptionCallback callback) {
    final senderKeyName = SenderKeyName(groupId, address);
    final groupCipher = GroupCipher(senderKeyStore, senderKeyName);
