...
class SignalProtocol {
  ...
  Future<Uint8List?> getSenderKeyPublic(
    String groupId,
    String userId, {
    String? sessionId,
  }) async {
    final senderKeyName = SenderKeyName(
      groupId,
      SignalProtocolAddress(userId, sessionId.getDeviceId()),
    );
  ...
  Future<SenderKeyDistributionMessageWrapper> getSenderKeyDistribution(
    ...
    final senderKeyName = SenderKeyName(
      groupId,
      SignalProtocolAddress(senderId, defaultDeviceId),
    );
  ...
  Future<EncryptResult> encryptSenderKey(
    ...
    final senderKeyDistributionMessage = await getSenderKeyDistribution(
      conversationId,
      _accountId,
    );
    try {
  ...
  Future<BlazeMessage> encryptGroupMessage(
    ...
    var cipher = <int>[];
    try {
      cipher = await groupCipher.encrypt(
        Uint8List.fromList(utf8.encode(message.content!)),
      );
    ...
    final data = encodeMessageData(
      ComposeMessageData(
        CiphertextMessage.senderKeyType,
